    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-6 py-16">
        ...
        <section className="py-16">
          <span className="font-mono text-xs uppercase tracking-widest text-white/50">[L] Liderazgo</span>
          <h2 className="mt-3 text-3xl font-bold text-white">Quienes dirigen el programa</h2>
          {!director && !subdirector ? (
            <p className="mt-6 text-white/50">Próximamente se anunciarán los líderes de este programa.</p>
          ) : (
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {director && (
                <div className="relative overflow-hidden rounded-xl">
                  <div className="absolute inset-x-0 top-0 h-0.5" style={{ backgroundColor: color }} />
                  <MemberCard name={director.name} role={director.role} bio={director.bio} socials={director.socials} />
                </div>
              )}
              {subdirector && (
                <MemberCard name={subdirector.name} role={subdirector.role} bio={subdirector.bio} socials={subdirector.socials} />
              )}
            </div>
          )}
          {activeMembers.length > 0 && (
            <div className="mt-10">
              <span className="font-mono text-xs uppercase tracking-widest text-white/50">[M] Miembros activos</span>
        ...
        <section className="border-t border-white/10 py-16">
          <div className="flex items-end justify-between">
            ...
            <Link to="/eventos" className="hidden rounded-md border border-white/20 px-3 py-1.5 text-sm text-white transition hover:bg-white/10 md:inline-block">
              Ver todos →
            </Link>
          </div>
          {events.length === 0 ? (
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {events.map((e) => <EventCard key={e.id} event={e} program={program} />)}
            </div>
          )}
        </section>
        ...
        <section className="border-t border-white/10 py-16">
          <div className="flex items-end justify-between">
            ...
            <Link to="/proyectos" className="hidden rounded-md border border-white/20 px-3 py-1.5 text-sm text-white transition hover:bg-white/10 md:inline-block">
              Ver todos →
            </Link>
          </div>
          {projects.length === 0 ? (
            <p className="mt-6 text-white/50">Sin proyectos registrados aún. ¡Pronto habrá novedades!</p>
          ) : (
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {projects.map((p) => <ProjectCard key={p.id} project={p} program={program} />)}
            </div>
          )}
        </section>
      </div>
